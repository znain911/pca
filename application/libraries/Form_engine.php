 ...
 * @author    		Sayeek Ahsan <sayeek10111@gmail.com>
 * @version   		1.0.0.1
...
class Form_engine {
     ...
     * @usage
     *      $coupon_form_options = $this->form_engine->auto_db_to_options($coupon, array(
     *           'code' => array(
     ...
     *
     * @usage
     *      $this->form_engine->change_pre_built($coupon_form_options, 'type', array(
     *           'help' => 'Percentage or Fixed Amount',
     *           'type' => 'dropdown',
     *           'options' => array(
     *               'P' => 'Percentage',
			...
			if (is_array($this->elm_options)) {
                ...
                switch ($this->func) {                	
					case 'form_hidden':
                        ...
                        break;
                    case 'form_header':
						$this->print_string .= $this->elm_options['html'];
						break;
					case 'form_checkbox':
                    case 'form_radio':
                        ...
                        foreach ($all_elm_options['options'] as $elm_suboptions) {
							...
							//$label_class = substr($this->func, 5).'-inline icheck';
							
							$label_class = isset($this->elm_options['class']) ? $this->elm_options['class'] : 'radio-inline';
                            $label_class .= ' icheck';
...
//        }
    ...
    private function _build_input($include_pre_post = true) {
        ...
        if ($this->func == 'form_combine') {
            ...
            foreach ($elm_options_backup['elements'] as $elm) {
				...
				//$input_html_string .= $this->_build_input();
                $counter++;
            }
            ...
            switch ($this->func) {
                ...
                case 'form_dropdown':
                    ...
                    if (isset($this->elm_options['options']) && !empty($this->elm_options['options'])) {
						...
						$theme = isset($this->elm_options['theme']) ? $this->elm_options['theme'] : '';
                ...
                case 'form_file':
                    ...
                    break;
                case 'form_checkbox':                    
					if(isset($this->elm_options['chktype'])){						
						switch ($this->elm_options['chktype']) {
							case 'normal':
								$input_html_string = form_checkbox($this->elm_options);	
								break;
							case 'onoff':
								$input_html_string = '<div data-on="primary" data-off="info" class="make-switch">'. form_checkbox($this->elm_options).'</div>';		
								break;
							case 'openclose':
								$input_html_string = '<div data-on-label="Open" data-off-label="Close" data-text-label="Door" class="make-switch">'. form_checkbox($this->elm_options).'</div>';		
								break;				
						}						
					}else{					
						$input_html_string = form_checkbox($this->elm_options);					
		...
		$ret_string .= ($include_pre_post) ? $this->_build_error_block( isset($this->elm_options['id']) ? $this->elm_options['id'] : '') : '';
	...
	private function _build_error_block($id) {
        return '<span class="form-error" id="error-'.$id.'"></span>';
        return '';
    }
